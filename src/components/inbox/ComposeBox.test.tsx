...
async function openRecommendationsMenu() {
  fireEvent.click(await screen.findByLabelText('Tambah lampiran atau template'))
  fireEvent.click(await screen.findByText('✨ Rekomendasi Template'))
}
...
describe('ComposeBox AI template recommendations', () => {
  const simpleOfficial = { id: 'tpl_simple', name: 'sapaan', type: 'OFFICIAL', category: null, body: 'Halo, ada yang bisa dibantu?', metaStatus: 'APPROVED', format: 'TEXT', variables: [] }
  const quickReply = { id: 'tpl_qr', name: 'Cara Booking', type: 'QUICK_REPLY', category: null, body: 'Ikuti panduan booking...' }

  function mockFetch(recommendResponse: unknown, templates: unknown[] = [simpleOfficial]) {
    return vi.fn((url: string, init?: RequestInit) => {
      if (url === '/api/templates') return Promise.resolve({ ok: true, json: async () => templates })
      if (url === '/api/templates/recommend' && init?.method === 'POST') {
        return typeof recommendResponse === 'function' ? recommendResponse() : Promise.resolve(recommendResponse)
      }
      return Promise.resolve({ ok: true, json: async () => ({ defaultChannel: 'OFFICIAL', id: 'm1', deliveryStatus: 'SENT' }) })
    })
  }

  it('requests recommendations for the current conversation and channel, and shows them with their reasons', async () => {
    const fetchMock = mockFetch({
      ok: true,
      json: async () => ({ recommendations: [{ templateId: 'tpl_simple', templateName: 'sapaan', reason: 'Pelanggan menyapa duluan' }] }),
    })
    vi.stubGlobal('fetch', fetchMock)
    render(<ComposeBox conversationId="conv_1" botEnabled={false} onSent={() => {}} onBotToggled={() => {}} />)

    await openRecommendationsMenu()

    expect(await screen.findByText('sapaan')).toBeInTheDocument()
    expect(screen.getByText('Pelanggan menyapa duluan')).toBeInTheDocument()
    expect(fetchMock).toHaveBeenCalledWith(
      '/api/templates/recommend',
      expect.objectContaining({ method: 'POST', body: JSON.stringify({ conversationId: 'conv_1', channel: 'OFFICIAL' }) })
    )
  })

  it('shows a loading state while waiting for the recommendation, then replaces it with results', async () => {
    ...
    expect(await screen.findByText('Menganalisis pesan masuk...')).toBeInTheDocument()

    resolveRecommend({
      ok: true,
      json: async () => ({ recommendations: [{ templateId: 'tpl_simple', templateName: 'sapaan', reason: 'cocok' }] }),
    })

    await waitFor(() => expect(screen.getByText('sapaan')).toBeInTheDocument())
    expect(screen.queryByText('Menganalisis pesan masuk...')).not.toBeInTheDocument()
  })

  it('shows an empty state when nothing relevant is recommended', async () => {
    vi.stubGlobal('fetch', mockFetch({ ok: true, json: async () => ({ recommendations: [] }) }))
    render(<ComposeBox conversationId="conv_1" botEnabled={false} onSent={() => {}} onBotToggled={() => {}} />)

    await openRecommendationsMenu()

    expect(await screen.findByText('Tidak ada template yang relevan untuk pesan ini.')).toBeInTheDocument()
  })

  it('shows an inline error when the recommend request fails', async () => {
    vi.stubGlobal('fetch', mockFetch({ ok: false, json: async () => ({ error: 'Tidak ada pesan masuk untuk dianalisis' }) }))
    render(<ComposeBox conversationId="conv_1" botEnabled={false} onSent={() => {}} onBotToggled={() => {}} />)

    await openRecommendationsMenu()

    expect(await screen.findByText('Tidak ada pesan masuk untuk dianalisis')).toBeInTheDocument()
  })

  it('closes the recommendations panel via the close button', async () => {
    vi.stubGlobal('fetch', mockFetch({ ok: true, json: async () => ({ recommendations: [] }) }))
    render(<ComposeBox conversationId="conv_1" botEnabled={false} onSent={() => {}} onBotToggled={() => {}} />)
    await openRecommendationsMenu()
    await screen.findByText('Tidak ada template yang relevan untuk pesan ini.')

    fireEvent.click(screen.getByLabelText('Tutup rekomendasi template'))

    expect(screen.queryByText('✨ REKOMENDASI TEMPLATE (AI)')).not.toBeInTheDocument()
  })

  it('clicking a recommended OFFICIAL template sends it immediately when it has no variables', async () => {
    const fetchMock = mockFetch({
      ok: true,
      json: async () => ({ recommendations: [{ templateId: 'tpl_simple', templateName: 'sapaan', reason: 'cocok' }] }),
    })
    ...
    render(<ComposeBox conversationId="conv_1" botEnabled={false} onSent={() => {}} onBotToggled={() => {}} />)
    await openRecommendationsMenu()
    await screen.findByText('sapaan')
  ...
  it('clicking a recommended quick reply pastes its body into the text input', async () => {
    vi.stubGlobal(
      'fetch',
      mockFetch(
        { ok: true, json: async () => ({ recommendations: [{ templateId: 'tpl_qr', templateName: 'Cara Booking', reason: 'cocok' }] }) },
        [quickReply]
      )
    )
    render(<ComposeBox conversationId="conv_1" botEnabled={false} onSent={() => {}} onBotToggled={() => {}} />)
    await switchToUnofficial()
    await openRecommendationsMenu()
    await screen.findByText('Cara Booking')

    fireEvent.click(screen.getByText('Cara Booking'))

    await waitFor(() => {
      expect(screen.getByPlaceholderText('Reply on WhatsApp...')).toHaveValue('Ikuti panduan booking...')
    })
  })
})
