...
export function CatalogEntryPanel({
  ...
  // Ketiga keadaan — memuat, kosong, dan berisi — memakai permukaan berbatas yang SAMA
  // (`TableContainer`, bentuk yang dipakai tabel Kontak). Sebelumnya daftar ini memakai garis
  // rambut atas/bawah tanpa wadah, jadi ia mengambang langsung di kanvas sementara tabel di
  // bagian bawah halaman yang sama duduk di dalam kotak.
  if (loading) {
    return (
      <TableContainer>
  ...
  return (
    <div className="space-y-2">
      ...
      {/* Daftar berpembatas garis rambut DI DALAM satu wadah, bukan tumpukan kartu: entri
          katalog adalah baris-baris sejenis, dan satu kotak per baris hanya menambah tinggi
          tanpa menambah arti. */}
      <TableContainer>
        <ul className="divide-y divide-line">
          {entries.map((entry) => {
            ...
            return (
              ...
              <li key={entry.id} className="grid gap-x-8 gap-y-1.5 px-4 py-3 xl:grid-cols-[minmax(0,1fr)_minmax(0,22rem)] xl:items-start">
