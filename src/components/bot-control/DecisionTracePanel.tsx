...
/**
 * Satu bagian trace. Labelnya kalimat biasa, bukan ALL-CAPS: sembilan label berhuruf besar
 * berjajar di panel selebar 24rem terbaca sebagai teriakan, bukan sebagai struktur.
 */
...
export function DecisionTracePanel({ run }: { run: DecisionRunDetail | null }) {
  ...
  if (!run) return <p className="text-base text-ink-muted">Trace tidak tersedia untuk pesan ini</p>
    ...
    <div className="space-y-3 text-base">
      <Section title="Keputusan akhir">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="font-mono text-xs text-ink-muted uppercase">{run.mode}</span>
          {run.latencyMs != null && <span className="font-mono text-xs text-ink-muted">{run.latencyMs} ms</span>}
          {run.flowKey && (
      ...
      <Section title="Pesan masuk">
        <p className="whitespace-pre-wrap text-ink">{run.inboundText}</p>
      </Section>

      {run.replyText && (
        <Section title="Balasan">
          <p className="whitespace-pre-wrap text-ink">{run.replyText}</p>
        </Section>
      )}
      ...
      {run.knowledgeRefs != null && (
        <Section title="Knowledge yang dipakai">
          <pre className="overflow-x-auto rounded-sm border border-line bg-surface-sunken p-2 text-xs text-ink">
            {JSON.stringify(run.knowledgeRefs, null, 2)}
          </pre>
        </Section>
      )}
      ...
      <div>
        ...
        {showRaw && (
          <pre className="mt-1 max-h-72 overflow-auto rounded-sm border border-line bg-surface-sunken p-2 text-xs text-ink">
