...
/**
 * POST /api/bot-control/channel-policy/approve — menyetujui draft kebijakan.
 *
...
export async function POST(req: Request) {
  ...
  if (!sessionCan(session, 'APPROVE')) {
    return NextResponse.json({ error: 'Peran Anda tidak boleh menyetujui draft kebijakan' }, { status: 403 })
  }
  ...
  try {
    ...
    console.error('POST /api/bot-control/channel-policy/approve gagal', error)
