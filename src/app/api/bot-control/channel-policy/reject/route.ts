...
/**
 * POST /api/bot-control/channel-policy/reject — menolak draft kebijakan.
 *
...
export async function POST(req: Request) {
  ...
  if (!sessionCan(session, 'APPROVE')) {
    return NextResponse.json({ error: 'Peran Anda tidak boleh menolak draft kebijakan' }, { status: 403 })
  }
  ...
  try {
    ...
    console.error('POST /api/bot-control/channel-policy/reject gagal', error)
