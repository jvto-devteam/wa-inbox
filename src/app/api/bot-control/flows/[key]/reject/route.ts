...
/**
 * POST /api/bot-control/flows/[key]/reject — menolak draft flow.
 *
...
export async function POST(req: Request, { params }: { params: Promise<{ key: string }> }) {
  ...
  if (!sessionCan(session, 'APPROVE')) {
    return NextResponse.json({ error: 'Peran Anda tidak boleh menolak draft flow' }, { status: 403 })
  }
  ...
  try {
    ...
    console.error('POST /api/bot-control/flows/[key]/reject gagal', error)
