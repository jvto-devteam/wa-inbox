...
/**
 * POST /api/bot-control/knowledge/sources/[id]/request-review — mengirim revisi ke review.
 *
 * Status only. The revision's content is untouched, and the bot does not read it until a
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!sessionCan(session, 'EDIT_KNOWLEDGE_DRAFT')) {
    return NextResponse.json({ error: 'Peran Anda tidak boleh mengirim revisi ke review' }, { status: 403 })
  }
  ...
  try {
    ...
    const result = await transitionKnowledge(
      id,
      'REVIEW',
    ...
    console.error('POST /api/bot-control/knowledge/sources/[id]/request-review gagal', error)
