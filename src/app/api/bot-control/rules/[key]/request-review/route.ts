...
/**
 * POST /api/bot-control/rules/[key]/request-review — mengirim draft ke review.
 *
...
export async function POST(req: Request, { params }: { params: Promise<{ key: string }> }) {
  ...
  if (!sessionCan(session, 'EDIT_RULE_DRAFT')) {
    return NextResponse.json({ error: 'Peran Anda tidak boleh mengirim draft ke review' }, { status: 403 })
  }
  ...
  try {
    ...
    console.error('POST /api/bot-control/rules/[key]/request-review gagal', error)
