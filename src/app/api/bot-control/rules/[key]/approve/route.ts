...
/**
 * POST /api/bot-control/rules/[key]/approve — menyetujui draft.
 *
...
export async function POST(req: Request, { params }: { params: Promise<{ key: string }> }) {
  ...
  if (!sessionCan(session, 'APPROVE')) {
    return NextResponse.json({ error: 'Peran Anda tidak boleh menyetujui draft' }, { status: 403 })
  }
  ...
  try {
    ...
    console.error('POST /api/bot-control/rules/[key]/approve gagal', error)
